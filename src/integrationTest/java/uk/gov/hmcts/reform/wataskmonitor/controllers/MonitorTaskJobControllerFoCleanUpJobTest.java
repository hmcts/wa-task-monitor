...
class MonitorTaskJobControllerFoCleanUpJobTest extends SpringBootIntegrationBaseTest {
    ...
    public void givenMonitorTaskJobRequestShouldReturnStatus200() throws Exception {
        MonitorTaskJobRequest monitorTaskJobReq = new MonitorTaskJobRequest(
            new JobDetails(JobName.CLEANUP_SENSITIVE_LOG_ENTRIES)
        );
        ...
        mockMvc.perform(post("/monitor/tasks/jobs")
            ...
            .andExpect(content().string(equalTo(expectedResponse.apply(JobName.CLEANUP_SENSITIVE_LOG_ENTRIES.name()))));
    ...
    private void mockExternalDependencies() {
        ...
        when(taskOperationClient.executeOperation(eq(SERVICE_TOKEN), Mockito.any(TaskOperationRequest.class)))
