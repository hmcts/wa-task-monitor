...
class MonitorTaskJobControllerFoReconfigurationFailuresJobTest extends SpringBootIntegrationBaseTest {
    ...
    public void givenMonitorTaskJobRequestShouldReturnStatus200() throws Exception {
        MonitorTaskJobRequest monitorTaskJobReq = new MonitorTaskJobRequest(
            new JobDetails(JobName.RECONFIGURATION_FAILURES)
        );
        ...
        mockMvc.perform(post("/monitor/tasks/jobs")
            ...
            .andExpect(content().string(equalTo(expectedResponse.apply(JobName.RECONFIGURATION_FAILURES.name()))));
