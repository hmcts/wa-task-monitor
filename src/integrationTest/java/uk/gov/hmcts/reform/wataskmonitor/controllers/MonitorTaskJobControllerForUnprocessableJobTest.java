...
class MonitorTaskJobControllerForUnprocessableJobTest extends SpringBootIntegrationBaseTest {
    ...
    public void shouldSucceedAndInitiateTasks() throws Exception {
        MonitorTaskJobRequest monitorTaskJobReq = new MonitorTaskJobRequest(new JobDetails(JobName.UNPROCESSABLE));

        mockMvc.perform(post("/monitor/tasks/jobs")
                .contentType(MediaType.APPLICATION_JSON)
                .content(TestUtility.asJsonString(monitorTaskJobReq)))
            .andExpect(status().isOk())
            .andExpect(content().string(equalTo(expectedResponse.apply(JobName.UNPROCESSABLE.name()))));

        verify(authTokenGenerator).generate();
        verify(caseEventHandlerClient).findProblematicMessages(
            eq(SERVICE_TOKEN),
            eq(JobName.UNPROCESSABLE.name())
        );

        verify(caseEventHandlerClient, times(1))
            .findProblematicMessages(eq(SERVICE_TOKEN), eq(JobName.UNPROCESSABLE.name()));
    }
